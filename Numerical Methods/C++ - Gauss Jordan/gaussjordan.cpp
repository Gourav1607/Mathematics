...
int main() {
	...
	for (int i = 0; i < n; i++) {		// Enter B (Augmented Matrix)
	...
	for (int i = 0; i < n; i++) {		// For Every Diagonal Element
		if (mat[i][i] == 0) {		// If The Index Is 0 Then Swap
			int temp = i;
			while (mat[temp][i] == 0) {	// Check Until Non-Zero Value Is Found
				temp++;
			}
			for (int j = 0; j < n && temp != i; j++) {// Swap With Non-Zero Valued Row
		...
		for (int j = 0; j < n; j++) {	// Dividing Row By Diagonal Element
			mat[i][j] /= a;
		}
		b[i] /= a;
		...
		for (int j = 0; j < n; j++) {	// For Every Row
			if (i != j && mat[j][i] != 0.0) {// Check If The Element Is Not Already Zero
											 // And It Is Not The Same As The Row With Diagonal Element
				a = mat[j][i];
				for (int k = 0; k < n; k++) {// For Every Element Of The Jth Row
	...
	cout << endl << " Final Augmented Matrix : " << endl;
	for (int i = 0; i < n; i++) {		// Print Matrix
		for (int j = 0; j < n; j++) {
			cout << " " << setw(SETW_DEF) << mat[i][j];
		}
